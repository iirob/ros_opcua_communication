...
class OpcUaROSAction:
    def __init__(self, server, parent, idx, name, action_type, feedback_type):
	  ...
	  self.main_node = self.parent.add_object(ua.NodeId(self.name.split("/")[-1], self.parent.nodeid.NamespaceIndex, ua.NodeIdType.String),
						ua.QualifiedName(self.name.split("/")[-1], self.parent.nodeid.NamespaceIndex))
        rospy.logdebug("Created Main Node under parent!")
        self.result = self.main_node.add_object(ua.NodeId(self.name + "_result", self.main_node.nodeid.NamespaceIndex, ua.NodeIdType.String),
        ...
        self.goal = self.main_node.add_object(ua.NodeId(self.name + "_goal", self.main_node.nodeid.NamespaceIndex, ua.NodeIdType.String),
        ...
        self.feedback = self.main_node.add_object(ua.NodeId(self.name + "feedback", self.main_node.nodeid.NamespaceIndex, ua.NodeIdType.String),
	...
	rospy.logdebug("Created feedback node")
        ...
        self.status = self.main_node.add_object(ua.NodeId(self.name + "status", self.main_node.nodeid.NamespaceIndex, ua.NodeIdType.String),
    ...
    def send_goal(self, parent, *inputs):
	    ...
	    rospy.loginfo("Created Message Instance for goal-send: " +  str(goal_msg))
            self.client.send_goal(goal_msg, done_cb=self.update_result, feedback_cb=self.update_feedback, active_cb=self.update_state)
	    return
	...
	rospy.loginfo("Creating message for goal call")
	already_set = []
	if isinstance(inputs, tuple):
	  ...
	  while(arg_counter < len(inputs) and object_counter < len(sample.__slots__)):
	   ...
	   while (cur_slot == 'header'):
	      rospy.loginfo("ignoring header")
	      object_counter+=1
	      if (object_counter < len(sample.__slots__)):
		cur_slot = sample.__slots__[object_counter]
	   real_slot = getattr(sample, cur_slot)
	   rospy.loginfo("cur_arg: " + str(cur_arg) + " cur_slot_name: " + str(cur_slot) + " real slot content: " + str(real_slot)) 
	   if hasattr(real_slot, '_type'):
	     rospy.loginfo("We found an object with name " + str(cur_slot) + ", creating it recursively")
	     arg_counter_before = arg_counter
	     already_set, arg_counter = self.create_object_instance(already_set, real_slot, cur_slot, arg_counter, inputs, sample)
	     if (arg_counter != arg_counter_before):  
	      object_counter += 1
	     rospy.loginfo("completed object, object counter: " + str(object_counter) + " len(object): " + str(len(sample.__slots__)))
	   else:
    ...
    def create_object_instance(self, already_set, object, name, counter, inputs, parent):
	  ...
	  while(object_counter < len(object.__slots__) and counter < len(inputs)):
	    ...
	    rospy.loginfo("cur_arg: " + str(cur_arg) + " cur_slot_name: " + str(cur_slot) + " real slot content: " + str(real_slot)) 
	    if hasattr(real_slot, '_type'):
	      rospy.loginfo("Recursive Object found in request/response of service call")
	      already_set, counter = self.create_object_instance(already_set, real_slot, cur_slot, counter, inputs, object)
	      object_counter += 1
	    else:
	       already_set.append(cur_slot)
