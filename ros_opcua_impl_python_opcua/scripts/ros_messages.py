...
class OpcUaROSMessage:
    ...
    def _recursive_create_items(self, parent, idx, package, message_type, message):
        ...
        if hasattr(message, '__slots__')  and hasattr(message, '_slot_types') :
            ...
            nodeIdentifier =  package+'/'+message_type
            if nodeIdentifier in ros_global.messageNode.keys() :
                node = ros_global.messageNode[nodeIdentifier]
            else :
                ...
                ros_global.messageNode[nodeIdentifier] =  new_node
                ros_global.dataTypeNode[nodeIdentifier]=  datatyp_node
...
def _getOrAddNewNodeVariableTypeRelatedToAMessage(message, server, idx) :
    ...
    node = None
    # if node is a  complex type, check if node messagenode exits -> add reference to the current messagenode if not create messagenode and add reference
