...
## Actions
- [x] Wrong Argument Parsing, as Int32 type arguments are wrongly parsed to Int16 ua.Arguments()
(See BlinkyAction, PoliceAction on SR2 Robot in Lab for example)

- [x] Move_Base_Simple action has special case as to fix Errors that were thrown, possibly it is a malformed Action Definition
```python
...
- [x] Actions are seen as Actions when */goal*, */feedback*, */result* or */status* is in the topic name, better heuristic possibly needed
    + No heuristics found in action management of ROS, ideal in terminal were `rosaction list` analog to `rostopic list`, but solved with checking at least two features of */goal*, */feedback*, */result* , */status* or */cancel*.

- [x] We ignore "header" in the goal call message instance to create our Arguments (and back the other way). Possible mistake See:
```python
...
```
with extension object, the problem is solved.
## Topics:

- [x] Better Idea for publishing OPC UA Data ?
    + Use Events to trigger while get a changed data. The client can subscribe the event and get informed.

## Services

- [x] Methods return value Strings are not correctly transfered to OPC UA somehow (maybe an UAExpert issue, as they can be printed in the call method onto the console correctly)
...
- [x] The HasTypeDefinition reference is capsuled to BasicDataVariableType for Variable, therefore the created VariableType can not be used for further modelling.

- [x] Use the low level and middle level functions in python OPC UA to create functions for meta-modelling of ROS.
...
- [x] Problem: If python opcua supports method call with customized variable types as arguments (The ROS messages we imported).
...
- [x] also called customized structure, create extension objects on the fly,
...
1. - [x] Actionlib uses unique goal ID to encrypt its topic publication. Problem of displaying the status of action client, if so, see [here](https://answers.ros.org/question/265723/actionlib-client-how-to-get-goal-id/)

2. - [x] Action client, how to model it?
